...
(function () {
  ...
  if (up==-1){
    socket.emit("revokedownvote",data);
    up=0;
  }
  $("#upnum").text(up);
  //socket.emit("upvote",data);
}
function ondownVote(data){
    if (up==0){
    socket.emit("downvote",data);
    up=-1;
  }
    if (up==1){
      socket.emit("revokeupvote",data);
      up=0;
    }
    $("#upnum").text(up);
    //socket.emit("downvote",data);
...
$(document).ready(function() {
  $('#show-poll-creation').click(function() {
    $('#poll-creation-section').toggle();
  });

  $('#add-option').click(function() {
    const optionNumber = $('#poll-options .poll-option').length + 1;
    $('#poll-options').append(`<input type="text" class="poll-option" placeholder="Option ${optionNumber}">`);
  });

  $('#create-poll-btn').click(function() {
    const question = $('#poll-question').val().trim();
    const options = $('.poll-option').map(function() { return $(this).val().trim(); }).get();
    ...
    socket.emit('createPoll', { question: question, options: options });
  ...
  socket.on('pollCreated', function(poll) {
    // Display poll UI
    $('#polls').html('<div id="poll">' +
                     '<p>' + poll.question + '</p>');
    poll.options.forEach(function(option, index) {
      $('#poll').append('<p><input type="radio" name="poll" value="' + index + '">' + option + '</p>');
    });
    $('#poll').append('<button id="submit-vote">Submit Vote</button>');
  });

  socket.on('votesUpdated', function(poll) {
    // Update vote counts in the poll UI
    $('#poll p').each(function(index) {
      if (index > 0) { // Skip question element
        $(this).text(poll.options[index - 1] + ': ' + poll.votes[index - 1] + ' votes');
      }
    });
  });
});
...
/*Signature Part */
$(document).ready(function() {
  // Function to get the user's name (replace this with your own method)
  function getUserName() {
      return "John Smith"; // Replace this with your method of fetching user name
  }

  // Show user's name on mouse move
  $('#whiteboard').mousemove(function(e){
      var userName = getUserName();
      $('#signature-display').text(userName).css({
          top: e.pageY + 10,
          left: e.pageX + 10
      }).show();
  });
